...
const long intervalESP = 10000; // interval sending to esp

unsigned long previousMillisDisplay;
const long intervalDisplay = 7000; //display interval
int displayType = 1;
...
void setup() {
  ...
  //  rtc.setTime(13, 2, 0);     // Set the time to 12:00:00 (24hr format)
  //  rtc.setDate(29, 12, 2018);   // Set the date to August 6th, 2010 (format = DD, MM, YYYY, eq 28,2,2018)
...
void loop() {
        ...
        if (atCommand("AT+CMGR=" + String(id), EMPTY, true)) {
          ...
          if (cp.startsWith("+639") || cp.startsWith("09")) {
            if (response.indexOf("airduino help") >= 0) {
              sendSMS(cp, "Send Possible Message Request");
            }
            else if (response.indexOf("get temp") >= 0) {
              sendSMS(cp, "Send Temperature: " + String(temp));
            }
            else if (response.indexOf("get humidity") >= 0) {
              sendSMS(cp, "Send Humidity: " + String(humidity));
            }
            else if (response.indexOf("get air") >= 0) {
              sendSMS(cp, "Send Air Quality: " + String(mqtVal));
            }
            else if (response.indexOf("get all") >= 0) {
              sendSMS(cp, "Send All");
            }
            else if (response.indexOf("get temp & air") >= 0) {
              sendSMS(cp, "temp and air" + String(temp) + ", " + String(mqtVal) );
            }
          }
        }
  ...
  if (currentMillis - previousMillisESP >= intervalESP) {
    ...
    if (sendESP(255)) {
      ...
      sendESP(humidity);
      sendESP(temp);
      sendESP(2); //1 = Imus, 2 = Dasma, 3 = Bacoor
    }
  }
}
